...
const fetchMission = async (userToken) => {
  const headers = {
  ...
  while (retryCount < maxRetries) {
    try {
      const response = await axios.get(
        "https://zero-api.kaisar.io/mission/tasks",
        {
          headers: headers,
      ...
      if (status === 200) {
        ...
        // Error response 2xx
        console.log(red(`Error encountered during fetchMission:`));
...
const claimMission = async (userToken, tasksIds) => {
    ...
    while (retryCount < maxRetries) {
      try {
        const response = await axios.post(
          `https://zero-api.kaisar.io/mission/tasks/${taskId}/claim`,
          null,
          {
            headers: headers,
            httpsAgent: proxyAgent,
        ...
        if (status === 200) {
          ...
          console.log(`Claimed Reward with Task Id: ${taskId}\n`);
          return;
        } else {
          // Error response 2xx
          console.log(red(`Error encountered during claimMission:`));
          console.log(red(`Status: ${status}`));
          console.log(red(`Message: ${data?.message || "Unknown error"}`));
...
const fetchAndClaimMission = async (userToken) => {
  const tasksIds = await fetchMission(userToken);

  if (tasksIds && tasksIds.length > 0) {
    console.log(
      `Found ${
        tasksIds.length
      } task(s) that can be claimed with IDs: ${tasksIds.join(", ")}`
    );
    await claimMission(userToken, tasksIds);
  } else {
    console.log(red("No active tasks available to claim at the moment...\n"));
  }
};

export { fetchAndClaimMission };
