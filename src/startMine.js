...
const startMine = async (nodeId, nodeToken) => {
  const payload = {
    extension: nodeId,
  };
  ...
  while (retryCount < maxRetries) {
    try {
      const response = await axios.post(
        "https://zero-api.kaisar.io/mining/start",
        payload,
        {
          headers: headers,
      ...
      if (status === 200) {
        ...
        // Error response 2xx
        console.log(red(`Error encountered during startMine:`));
      ...
      if (error.response) {
        ...
        if (status === 412) {
          console.log(red("Mining is already in progress...\n"));
          return;
        }
